...
extension FNMNetworkMonitorURLProtocol: URLSessionDataDelegate {
    ...
    func urlSession(
        ...
        let mertrics: [TimeInterval] = metrics.transactionMetrics.compactMap {
            guard let responseDataCount = dataDownloaded?.count,
                  responseDataCount > 0,
                  let start = $0.responseStartDate,
                  let end = $0.responseEndDate
            else { return nil }

            let duration = end.timeIntervalSince1970 - start.timeIntervalSince1970
            return duration > 0 ? duration : nil
        }
        //print("[Network Monitor] Longest: \(mertrics.max()) \(request.url)")
        requestRecord?.responseLoadingTime = mertrics.max()
    }
}
...
extension FNMNetworkMonitorURLProtocol {
    ...
    func concludeRecordIfNeeded(conclusion: FNMHTTPRequestRecordConclusionType) {
        ...
        if let requestRecord = self.requestRecord,
            ...
            requestRecord.endTimestamp = CACurrentMediaTime()
