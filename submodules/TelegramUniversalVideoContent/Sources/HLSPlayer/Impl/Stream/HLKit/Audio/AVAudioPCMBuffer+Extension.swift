...
extension AVAudioPCMBuffer {
    final func makeSampleBuffer(_ when: AVAudioTime) -> CMSampleBuffer? {
        ...
        status = CMAudioSampleBufferCreateWithPacketDescriptions(
            ...
            presentationTimeStamp: when.makeCMTime(),
            packetDescriptions: nil,
            sampleBufferOut: &sampleBuffer
        )
        guard let sampleBuffer else {
            print("[Warning]CMAudioSampleBufferCreateWithPacketDescriptions returned error: ", status)
            return nil
        }
        ...
        if status != noErr {
            print("[Warning] CMSampleBufferSetDataBufferFromAudioBufferList returned error: ", status)
        }
        return sampleBuffer
    }
